...
字段级 `@EntityDef`（继承 `mindustry.gen.UnitEntity`）可作为 `UnitType` 实体类型，用 `EntityRegistry.content(name, MyUnitEntity::class.java) { UnitType(it) }` 绑定（见 [5.8](../reference/generated-code.md#_5-8-实体注册-entityregistry-kt)）。
