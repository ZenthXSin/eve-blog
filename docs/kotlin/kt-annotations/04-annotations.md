...
- `pooled` / `genio` / `legacy` / `excludeGroups`：当前版本已解析但**尚未参与生成逻辑**（见 [已知限制](./11-limitations.md)）。
...
- 类级与字段级实体都会生成：
  ...
  - 统一注册入口 `EntityRegistry`（见 [5.8](./05-generated-code.md#_5-8-实体注册-entityregistry-kt)）。
