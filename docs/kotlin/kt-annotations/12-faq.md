...
A：`v159.7.2` 起可以：字段级 `@EntityDef` 的 `extends` 默认指向 `mindustry.gen.UnitEntity`（也可自定义基类），生成的类直接继承原版实体、可作为 `UnitType` 实体类型，用 `EntityRegistry.content(...)` 绑定（见 [5.8](./05-generated-code.md#_5-8-实体注册-entityregistry-kt)）。代价是此时实体字段/方法来自继承而非组件合并，本地组件的字段仍会合并进子类。
