...
- **重要**：生成代码会给接口/实体打上 `@EntityInterface` 注解，该注解引用 `genPackage` 包下的 `EntityInterface` —— 消费模块必须自己在 gen 包中声明一个（见 [8.3](./08-plugin-config.md#_8-3-genpackage-与-entityinterface)）。
